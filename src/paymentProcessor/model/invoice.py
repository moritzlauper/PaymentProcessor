...
class Invoice:
    ...
    def to_txt(self, model):
        ...
        txt_string = '%s\n%s\n%s\n%s\n%s\n\n\n\n\n%s, den %s\t\t\t\t\t\t%s\n\t\t\t\t\t\t\t\t\t\t\t%s\n\t\t\t\t\t\t\t\t\t' \
                     '\t\t%s' \
                     % (self.e(model[1][2]), self.e(model[1][3]), self.e(model[1][4]), self.e(model[1][5]), self.e(model[1][6])
        ...
        txt_string += '\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\nZahlungsziel ohne Abzug{0:3} Tage ({1})\n\nEinzahlungsschein' \
            .format(int(self.e(model[0][5])), '{0}.{1}.{2}'.format(date_string[6:8], date_string[4:6], date_string[0:4]))
